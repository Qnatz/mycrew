...
from .agent import otp_verifier_agent # Import the specific agent instance
...
def run_otp_verifier_agent_task(task_description: str):
    """
    Runs a task using the otp_verifier_agent with retry logic.
    """
    print(f"Initializing task for OTP Verifier Agent with description: '{task_description}'")

    # Define the task for the OtpVerifierAgent
    agent_task = Task(
        description=task_description,
        expected_output="Task output, typically a status message indicating OTP validation success or failure.",
        agent=otp_verifier_agent
    )

    # Create a crew for the OtpVerifierAgent
    agent_crew = Crew(
        agents=[otp_verifier_agent],
        tasks=[agent_task],
        llm=default_llm,
        verbose=True
    )

    print("\nKicking off OTP Verifier Agent task...")
    max_retries = 3
    attempts = 0
    last_exception = None
    ...
    while attempts < max_retries:
        try:
            ...
            print("\nOTP Verifier Agent Task Complete.")
            print("--------------------------------------")
            print("Result from OTP Verifier Agent:")
            if result:
                print(result.raw if hasattr(result, 'raw') else str(result))
            else:
                print("OTP Verifier Agent produced no output.")
            print("--------------------------------------")
            return result
        except Exception as e:
            attempts += 1
            last_exception = e
            print(f"\nAn error occurred during OTP Verifier Agent task execution (Attempt {attempts}/{max_retries}): {e}")
    ...
    error_message = f"OTP Verifier Agent task failed after {max_retries} attempts."
    if last_exception:
...
if __name__ == "__main__":
    # Example usage:
    sample_task = "Verify the OTP '123456' for user 'user@example.com' associated with session ID 'session123'."
    print(f"Running OTP Verifier Agent with sample task: '{sample_task}'")

    result = run_otp_verifier_agent_task(sample_task)

    print("\n\n####################################################")
    print("## OTP Verifier Agent Main Execution Result:")
    print("####################################################\n")
    if result and not isinstance(result, str):
        print("Final output from the OTP Verifier Agent task:")
        print(result.raw if hasattr(result, 'raw') else str(result))
    elif isinstance(result, str):
        print(f"Task execution failed or produced a message: {result}")
    else:
        print("OTP Verifier Agent task produced no definitive output or an error occurred.")
